...
export async function createGPTPromptLists() {
    ...
    // We will start with 80% examples for tunning and 20% for validation.
    const tunningPromptsSize = Math.floor(0.8 * prompts.length);
    // const validationPromptsSize = prompts.length - tunningPromptsSize;
    const tunningPrompts = prompts.slice(0, tunningPromptsSize);
    const validationPrompts = prompts.slice(tunningPromptsSize);
